...
public class CercleDaoJDBC extends AbstractDao<Cercle> {
	...
	public Cercle create(final Cercle c) {
		try {
			...
			prepare = connect
					.prepareStatement("INSERT INTO Cercle (Nom,Centre_X,Centre_Y," + "Rayon)" + "VALUES (?,?,?,?)");
			prepare.setString(un, c.getNom());
			...
			if (result.next()) {
				try {
					c = new Cercle(result.getString("Nom"),
							new Position(result.getInt("Centre_X"), result.getInt("Centre_Y")), result.getInt("Rayon"));
				} catch (Exception e) {
					e.printStackTrace();
					return null;
				}
		...
		if (c2 != null) {
			try {
				...
				PreparedStatement prepare = connect
						.prepareStatement("UPDATE Cercle SET Centre_X = ?," + "Centre_Y = ?, Rayon = ? WHERE Nom = ?");
