	...
	public Rectangle create(final Rectangle r) {
		try {
			...
			prepare = connect.prepareStatement(
					"INSERT INTO Rectangle (Nom,Centre_X,Centre_Y," + "Longueur, Hauteur)" + "VALUES (?,?,?,?,?)");
			prepare.setString(un, r.getNom());
			...
			if (result.next()) {
				try {
					r = new Rectangle(result.getString("Nom"),
							new Position(result.getInt("Centre_X"), result.getInt("Centre_Y")),
							result.getInt("Longueur"), result.getInt("Hauteur"));
				} catch (Exception e) {
					e.printStackTrace();
					return null;
				}
	...
	public Rectangle update(final Rectangle r) {
		...
		if (r2 != null) {
			try {
				...
				final int quatre = 4;
				final int cinq = 5;
				PreparedStatement prepare = connect.prepareStatement("UPDATE Rectangle SET Centre_X = ?,"
						+ "Centre_Y = ?, Longueur = ?, Hauteur = ?" + " WHERE Nom = ?");
