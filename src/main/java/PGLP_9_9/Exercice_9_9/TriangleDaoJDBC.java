...
public class TriangleDaoJDBC extends AbstractDao<Triangle> {
	...
	public Triangle create(final Triangle t) {
		try {
			...
			prepare = connect.prepareStatement("INSERT INTO Triangle (Nom,Sommet1_X,Sommet1_Y,"
					+ "Sommet2_X,Sommet2_Y,Sommet3_X,Sommet3_Y)" + "VALUES (?,?,?,?,?,?,?)");
			prepare.setString(un, t.getNom());
		...
		try {
			...
			if (result.next()) {
				t = new Triangle(result.getString("Nom"),
						new Position(result.getInt("Sommet1_X"), result.getInt("Sommet1_Y")),
						new Position(result.getInt("Sommet2_X"), result.getInt("Sommet2_Y")),
						new Position(result.getInt("Sommet3_X"), result.getInt("Sommet3_Y")));
			}
		} catch (SQLException e) {
			e.printStackTrace();
			return null;
